...
If you've installed `emacs-mac` via `homebrew`, `emacsclient` will be in `/usr/local/bin`. Note that the mac port of Emacs currently does not support MultiTTY and as such, `emacsclient -t` will not work as expected.
